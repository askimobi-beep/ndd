...
export default function AgentPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const data = await apiRequest("/api/users/agents", {
        method: "POST",
        body: JSON.stringify({
      ...
      setNotification({ text: data.message || "Agent created successfully", type: "success" });
      setIsModalOpen(false);
      resetForm();
      await loadAgents();
    } catch (error) {
      setNotification({ text: error.message || "Unable to create agent", type: "error" });
    } finally {
  ...
  const handleEditSubmit = async (event) => {
    ...
    try {
      ...
      setNotification({ text: data.message || "Agent updated successfully", type: "success" });
      setIsEditOpen(false);
    } catch (error) {
      setNotification({ text: error.message || "Unable to update agent", type: "error" });
    } finally {
  ...
  const toggleStatus = async (record) => {
    try {
      ...
      setNotification({
        text: data.user?.isActive ? "Agent activated successfully" : "Agent blocked successfully",
        type: "success",
      });
    } catch (error) {
      setNotification({ text: error.message || "Unable to update agent status", type: "error" });
    }
  };
        ...
        <div className="w-full space-y-8">
          ...
          <div className="rounded-[28px] border border-white/70 bg-white p-6 shadow-[0_14px_40px_rgba(15,23,42,0.08)] lg:p-8">
            <div className="flex items-center justify-between">
              <h3 className="text-2xl font-semibold text-slate-900">Agent Table</h3>
      ...
      {isEditOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/55 px-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
            <div className="flex items-center justify-between">
              <h3 className="text-2xl font-semibold text-slate-900">Edit Agent</h3>
              <button
